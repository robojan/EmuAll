...
namespace robojan {
	...
	unsigned int WavFileInStream::ReadFloat(float *buffer, unsigned int samples)
	{
		...
		if (_waveFormatInfo->GetCompression() == WavFmtChunk::IEEE_FLOAT && _waveFormatInfo->GetBitsPerSample() == 32 && isHostLittleEndian())
	...
	unsigned int WavFileInStream::ReadPCM16(int16_t *buffer, unsigned int samples)
	{
		...
		if (_waveFormatInfo->GetCompression() == WavFmtChunk::PCM && _waveFormatInfo->GetBitsPerSample() == 16 && isHostLittleEndian())
			return Read(buffer, samples); // use native
	...
	unsigned int WavFileOutStream::WritePCM8(uint8_t *buffer, unsigned int samples)
	{
		...
		if (_waveFormatInfo->GetCompression() == WavFmtChunk::PCM && _waveFormatInfo->GetBitsPerSample() == 8 && isHostLittleEndian())
		{
			return Write(buffer, samples);
		}
		/*
